    ...
    <section className="py-20 lg:py-28">
      <div className="container mx-auto px-5 sm:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {phases.map((phase) => (
            <div key={phase.step} className="rounded-xl bg-card border border-border p-6 border border-border/40">
              <span className="text-xs text-primary font-medium">{phase.step}</span>
              <h3 className="text-lg font-medium text-foreground mt-2 mb-3">{phase.title}</h3>
