...
export function TestimonialsBlock({ title = 'What Our Clients Say', testimonials = [] }: Props) {
  ...
  return (
    <section className="py-16 px-6 lg:px-10 max-w-6xl mx-auto">
      <h2 className="text-2xl font-medium text-foreground mb-10 text-center">{title}</h2>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {testimonials.map((t, i) => (
          <div key={i} className="rounded-xl bg-card border border-border border border-border/60 p-6 flex flex-col gap-4">
