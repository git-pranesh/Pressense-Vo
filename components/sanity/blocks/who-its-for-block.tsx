  ...
  return (
    <section className="py-16 px-6 lg:px-10 max-w-5xl mx-auto">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-10">
        {scenarios.map((s, i) => (
          <div key={i} className="rounded-xl bg-card border border-border border border-border/60 p-5">
            {s.icon && <span className="text-2xl mb-3 block">{s.icon}</span>}
            <p className="text-sm font-medium text-foreground mb-1.5">{s.label}</p>
      ...
      {notForItems.length > 0 && (
        <div>
          <p className="text-sm font-medium text-muted-foreground mb-3">{notForTitle || 'This is NOT for you if...'}</p>
